...
{
    public class GlobalRouteValuesLinkGeneratorTemplateBinder
    {
        ...
        /// <summary>
        /// Creates a new instance of <see cref="GlobalRouteValuesLinkGeneratorTemplateBinder"/>.
        /// </summary>
        ...
        internal GlobalRouteValuesLinkGeneratorTemplateBinder(
        ...
        public TemplateValuesResult GetValues(RouteValueDictionary ambientValues, RouteValueDictionary values)
        {
                ...
                for (var i = 0; i < requiredKeys.Length; i++)
                {
                    ...
                    if (!hasParameter)
                    {
                        ...
                        if (!RoutePartsEqual(ambientValue, _pattern.RequiredValues[key]) &&
                            !RoutePatternIsRequiredValueAny(_pattern.RequiredValues[key]))
                        {
            ...
            for (var i = 0; i < parameterCount; i++)
            {
                ...
                if (!copyAmbientValues && !hasExplicitValue && _pattern.RequiredValues.TryGetValue(key, out var requiredValue))
                {
                    hasAmbientValue = ambientValues != null && ambientValues.TryGetValue(key, out ambientValue);
                    if (hasAmbientValue &&
                        (RoutePartsEqual(requiredValue, ambientValue) || RoutePatternIsRequiredValueAny(requiredValue)))
                    {
        ...
        //
        // Processes the constraints **if** they were passed in to the GlobalRouteValuesLinkGeneratorTemplateBinder constructor.
